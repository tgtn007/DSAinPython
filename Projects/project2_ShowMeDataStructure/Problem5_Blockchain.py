...
class Blockchain():
    ...
    def __iter__(self):
        node = self.head
        while node:
            yield ['Data :{}, SHA256 Hash: {}, Previous Hash: {}, Timestamp: {}'.format(node.data, node.hash,
                                                                                        node.previous_hash,
                                                                                        node.timestamp)]
            node = node.next
...
def test(newblocks):
    current = Blockchain()
    for data in newblocks:
        prev = current.get_last_hash()
        new_block = current.add_block(data, prev)
        if new_block != None:
            print("Adding new block: {}".format(new_block.data))
            print("TimeStamp: {}".format(new_block.timestamp))
            print("Hash: {}".format(new_block.hash))
            print("Previous Hash: {}".format(prev))
            print("\n")
        else:
            print("Block with value: {} not added".format(data))
            print("\n")
    if current.is_chain_valid():
...
test([10, 100, 100, 80, '', 2])  # Expected output: Pass: blockchain is Valid
test([100000, 20, None, 2])  # Expected output: Pass: blockchain is Valid
